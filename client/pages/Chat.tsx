...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    if (location.state?.openDiscovery) {
      setIsDiscoveryOpen(true);
      navigate(location.pathname, { replace: true, state: {} });
    }
    if (location.state?.view === 'contacts') {
      ...
      <div className="flex flex-col items-center justify-center h-screen p-8 text-center bg-white dark:bg-[#0d1418] overflow-y-auto">
        ...
        {localStorage.getItem("pending_otp") === "true" ? (
          <div className="w-full max-w-xs space-y-3 shrink-0">
            <button onClick={() => navigate("/verification-code")} className="w-full py-4 bg-vic-green text-slate-900 font-bold rounded-2xl shadow-lg">
              Masukkan Kode
            </button>
            <button onClick={() => { localStorage.removeItem("pending_otp"); navigate("/phone-input"); }} className="w-full py-4 bg-transparent border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 font-bold rounded-2xl">
              Ganti Nomor Telepon
            </button>
          </div>
        ) : (
          <button onClick={() => navigate("/phone-input")} className="w-full max-w-xs py-4 bg-vic-green text-slate-900 font-bold rounded-2xl shadow-lg shrink-0">
            {t('verify_phone')}
          </button>
        )}
      </div>
      ...
      <main className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="divide-y divide-slate-50 dark:divide-white/[0.02]">
            ...
            <button
              onClick={() => selfConv ? navigate(`/chat/${selfConv.id}`) : openSelfChat()}
