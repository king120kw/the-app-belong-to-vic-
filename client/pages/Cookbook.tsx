        ...
        <div className="flex flex-col h-screen max-w-2xl mx-auto w-full bg-white dark:bg-[#0d1418]">
            ...
            <header className="p-4 bg-white dark:bg-[#0d1418] sticky top-0 z-10">
                <div className="flex items-center justify-between mb-4">
                    <Link to="/dashboard" className="text-vic-deep-blue dark:text-vic-green">
...
function CookbookCard({ item, type = 'recipe' }: { item: any, type?: 'recipe' | 'meal' }) {
    ...
    return (
        <Link to={`/recipe/${id}`} state={{ recipe: item }} className="block relative h-48 rounded-[32px] overflow-hidden group shadow-sm hover:shadow-md transition-shadow">
            {/* Background Image */}
