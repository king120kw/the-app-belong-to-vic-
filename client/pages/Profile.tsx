  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-white dark:bg-[#0d1418] p-6 text-center">
        ...
        <button onClick={() => navigate("/auth")} className="px-6 py-3 bg-vic-deep-blue text-white rounded-lg font-bold">{t('sign_in')}</button>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-screen max-w-2xl mx-auto w-full bg-white dark:bg-[#0d1418]">
      ...
      <header className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10 bg-white dark:bg-[#0d1418]">
        ...
        <button onClick={() => navigate("/settings")} className="text-slate-600 dark:text-slate-400">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="relative mb-6 group">
            <div
              ...
              style={{ backgroundImage: `url("${(profile as any)?.avatar_url || user.user_metadata?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent((profile as any)?.full_name || user.user_metadata?.full_name || 'User')}&background=13ec37&color=fff&size=256`}")` }}
            >
              <div
